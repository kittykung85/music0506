...
tile1.helpers = {
    ...
    handleDragLeave1: function() {
        $(this).removeClass(tile1.classes.draggingOver);
        console.log("handleDragLeave");
    },

    handleDrop1: function(e) {
        var $droppedOnElement = $(this),
            indexd,
            $temp;
    ...
    handleDragEnd1: function(elements, classesToRemove) {
        elements.removeClass(classesToRemove);
        console.log("handleDragEnd");
...
$(function() {
    // Attaches drag events
    tile1.elements.$containerTiles.on( 'dragstart', '.tile1', tile1.helpers.handleDragStart1 )
        .on( 'dragenter', '.tile1', tile1.helpers.handleDragEnter1 )
        .on( 'dragover' , '.tile1', tile1.helpers.handleDragOver1  )
        .on( 'dragleave', '.tile1', tile1.helpers.handleDragLeave1 )
        .on( 'drop'     , '.tile1', tile1.helpers.handleDrop1      )
...
function mergeAudio(){
    ...
    console.log(q);
    //var myAudio = document.getElementById(o+ "Audio");
    var playlist = new Array('./musicp/CMajor/CM_piano/'+o+'.wav','./musicp/CMajor/CM_piano/'+p+'.wav','./musicp/CMajor/CM_piano/'+q+'.wav');
    let PlaylistString = playlist.toString();
    console.log(PlaylistString); //轉字串
    if (PlaylistString.includes('./musicp/CMajor/CM_piano/e.wav,./musicp/CMajor/CM_piano/a.wav')){
        var yn= confirm('no');
        if (yn==true){
            audio.addEventListener('ended', function () {
                i = ++i < playlist.length ? i : 0;
